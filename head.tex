...
\numberwithin{equation}{subsection} % 式番号をサブセクション単位でリセット
...
%   opacity=0.25,                   % 透かしの透明度（0が完全透明、1が完全不透明）
%   angle=0,                       % 画像の角度
%   position = current page.center,  % ページの中央に配置
%   contents={\includegraphics{./fig/appilogo-circular-full.png}} % 画像のパス
% }
