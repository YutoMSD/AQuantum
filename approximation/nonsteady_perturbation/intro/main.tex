...
\begin{document}
  ...
  \begin{enumerate}
    \item $\hat{H} = \hat{H}^{(0)}$の場合\footnote{$\hat{H}^{(0)}$は厳密に解けるハミルトニアン．}\par
      ...
      \refe{psi-t-expantion}を\refe{time-dependent-schrodinger-eq}に代入すると，$\hat{H}$が時間に依存しないことに注意すれば，
      \begin{align}
  ...
  \refe{time-dependent-schrodinger-eq}と\refe{nonsteady-perturbation-cnt-eq}をまとめて，Schr\"odinger表示の非定常摂動基本方程式と呼ぶ．
\end{document}
